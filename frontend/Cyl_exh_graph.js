(function () {
    ...
    function getBrowserTimeZoneName() { try { return Intl.DateTimeFormat().resolvedOptions().timeZone || "Local"; } catch { return "Local"; } }
    ...
    function getSelectedCylinderLabels() {
        return Array.from(document.querySelectorAll('input[name="cyl-label"]:checked'), (input) => input.value);
    }
    function getCylinderDisplayLabel(label) {
        ...
        tooltipEl.innerHTML = `
            ...
            ${(tooltip.dataPoints || []).map((item) => `
                <div style="display:flex;align-items:center;gap:8px;">
                    ...
                    <span>${item.dataset.label}: ${item.formattedValue} ${item.dataset.unit || "deg C"}</span>
                </div>
            `).join("")}
        `;
            ...
            options: {
                ...
                scales: {
                    ...
                    y: {
                        ...
                        title: { display: true, text: "Exhaust Temp (deg C)", color: "#334155", font: { size: 14, style: "italic", weight: "700" } },
                        grid: { color: "rgba(100, 116, 139, 0.14)" },
                        border: { color: "rgba(100, 116, 139, 0.28)" },
                    },
                },
    ...
    function applyChartData(payload) {
        ...
        const selectedLabels = new Set(getSelectedCylinderLabels());
        ...
        state.chart.data.datasets = (Array.isArray(payload?.series) ? payload.series : []).flatMap((item) => {
            if (!selectedLabels.has(item?.label)) return [];
            ...
            const style = CONFIG.seriesStyle[item.label] || CONFIG.seriesStyle["No.1CYL. EXHAUST GAS TEMPERATURE"];
            return [{
                label: getCylinderDisplayLabel(item.label), unit: item.unit || "deg C", data, parsing: false, normalized: true,
                ...
                pointRadius: validPoints.length <= 2 ? 4 : 1.5, pointHoverRadius: 4, pointHitRadius: 10, borderWidth: 2.2, tension: 0.15,
                fill: false, clip: false, showLine: true, spanGaps: false,
            }];
        });
        state.chart.update("none");
    ...
    function getTrendParams() {
        const from = parseInputValueForTimeZone(DOM.from.value, state.timeZone);
        const to = parseInputValueForTimeZone(DOM.to.value, state.timeZone);
        const dgName = String(DOM.dg?.value || "").trim();
        if (!from || !to) return { error: "Please choose both From and To time." };
        if (from >= to) return { error: "'From' must be earlier than 'To'." };
        if (!dgName) return { error: "Please choose one DG." };
        if (getSelectedCylinderLabels().length === 0) return { error: "Please select at least one cylinder." };
        const params = new URLSearchParams({ from: from.toISOString(), to: to.toISOString(), dg_name: dgName, graph_type: "cylinder_exh", max_points: String(CONFIG.requestMaxPoints) });
        return { params };
    }
    async function loadTrend() {
        ...
        if (cachedPayload) {
            ...
            setStatus(`Loading cylinder exhaust trend for ${DOM.dg.value}...`);
        }
        try {
            const response = await fetchWithTimeout(`${CONFIG.apiBase}?${queryString}`, CONFIG.requestTimeoutMs, { cache: "no-store" });
            if (!response.ok) throw new Error(`Cylinder exhaust API error: ${response.status}`);
            const payload = await response.json();
            if (requestId !== state.activeRequestId) return;
            writeCachedPayload(queryString, payload);
            const totalVisiblePoints = applyChartData(payload);
            setStatus(totalVisiblePoints > 0 ? `${DOM.dg.value} cylinder exhaust trend loaded successfully.` : `No cylinder exhaust data for ${DOM.dg.value} in the selected time range.`, totalVisiblePoints === 0);
        } catch (error) {
    ...
    function bindEvents() {
        ...
        DOM.dg.addEventListener("change", loadTrend);
        document.querySelectorAll('input[name="cyl-label"]').forEach((input) => input.addEventListener("change", loadTrend));
    }
    function init() {
        if (DOM.dg) DOM.dg.value = initialDg;
        syncDefaultRange();
        updateHeaderTime();
